...
class xhetGenerator:
    ...
    def writeNamedSamples(self, total, outPath, names=None, header=True, digits=6):
        # write sample_xhet_gender.txt (use dummy names and supplied/inferred genders if needed)
        ...
        for i in range(len(samples)): 
            ...
            else: name = prefix+("%05d" % (i+1, )) # pad sample number to 5 digits
            if components[i]==0: gender = 1 # male
            elif components[i]==1: gender = random.choice((1,2)) # sampled from 'ambiguous' population
            else: gender = 2 # female
            output = "\t".join((name, str(round(samples[i], digits)), 'NA', str(gender) ))+"\n"
            out.write(output)
        out.close()
