...
my $uid = `whoami`;
chomp($uid);
my $session_log = user_session_log($uid, 'update_expression_metadata');
...
sub run {
  ...
  foreach my $data_object (@gex_data) {
    try {
      my $gdo = WTSI::NPG::Expression::InfiniumDataObject->new
        ($irods, $data_object);
      $gdo->update_secondary_metadata($ssdb);
      ++$updated;
...
Searches for published gene expression experimental data in iRODS,
identifies the plate from which it came by means of the gex_plate and
gex_well metadata and adds relevant sample metadata taken from the
Sequencescape warehouse. If the new metadata include study
